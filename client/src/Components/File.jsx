...
const File = ({ user }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await httpClient.post('http://localhost:8080/upload-file', formData, {
  ...
  const handleSearch = async () => {
    try {
      const response = await httpClient.get(`http://localhost:8080/search-files?query=${searchQuery}`);
